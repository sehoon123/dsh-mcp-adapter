...
export class MetadataCache {
  ...
  update(server, tools) {
    ...
    const summaries = tools.map((tool) => ({
      ...
      // The full input schema is cached so a `directTools`-promoted tool can be
      // registered with its REAL parameters at boot, before any server connects.
      // Oversized schemas are omitted (the tool then falls back to a freeform
      // args object) so one pathological server cannot bloat the cache file.
      ...serializableSchema(tool.inputSchema),
    }));
...
/** Cap on one cached input schema; larger ones are omitted. */
const MAX_SCHEMA_CHARS = 8 * 1024;

/**
 * Return `{inputSchema}` when the schema is small and JSON-safe, else `{}`.
 * @param {unknown} schema - the tool's published input schema.
 * @returns {{inputSchema?: object}} the cacheable fragment.
 */
function serializableSchema(schema) {
  if (!schema || typeof schema !== 'object' || Array.isArray(schema)) return {};
  try {
    const text = JSON.stringify(schema);
    if (!text || text.length > MAX_SCHEMA_CHARS) return {};
    return { inputSchema: JSON.parse(text) };
  } catch {
    // A cyclic or non-serializable schema is simply not cached.
    return {};
  }
}
...
function fingerprintTools(summaries) {
  return summaries
    .map((t) => `${t.name}:${t.description.length}:${t.required.join(',')}:${t.inputSchema ? JSON.stringify(t.inputSchema).length : 0}`)
    .sort()
    .join('|');
}
