...
export function configSearchPaths({ home = homedir(), cwd = process.cwd(), dshHome } = {}) {
  ...
  return [
    ...
    // A borrowed Pi config is a COMPATIBILITY source, not an authority: it must
    // rank BELOW this harness's own file. With the order reversed, a server also
    // present in ~/.pi/agent/mcp.json silently replaced the DSH entry and dropped
    // its DSH-only fields (observed: burp-suite lost `directTools: true`).
