...
export async function apply(ctx, rawConfig = {}) {
  ...
  // Operator visibility into what the proxy is hiding from the model.
  registerMcpCommand(ctx, runtime);
...
/**
 * Register the human-facing `/mcp` command.
 *
 * The proxy deliberately hides tool schemas from the model, which also hides them
 * from the OPERATOR. This command restores that visibility: which servers are
 * configured, which are connected or cooling behind an open circuit breaker, how
 * many tools each exposes, and which are promoted as direct tools.
 *
 * Reached with `ctx.get('commands')` rather than `inject`, so a profile without a
 * command registry simply does not get the command.
 *
 * @param {object} ctx - the plugin context.
 * @param {object} runtime - `{ manager, cache, config }`.
 */
function registerMcpCommand(ctx, runtime) {
  const commands = ctx.get('commands');
  if (!commands) return;
  const { manager, cache } = runtime;

  commands.register({
    name: 'mcp',
    description: 'MCP adapter status: configured servers, connections, and cached tool counts',
    handler: async (invocation) => {
      const arg = String(invocation?.rawInput ?? '').trim().toLowerCase();
      try {
        if (arg === 'refresh') {
          const names = manager.serverNames;
          const results = await Promise.allSettled(names.map((name) => manager.listTools(name)));
          const ok = results.filter((r) => r.status === 'fulfilled').length;
          return { text: `Refreshed ${ok}/${names.length} MCP server catalog(s).` };
        }
        return { text: renderStatus(manager, cache) };
      } catch (error) {
        // A status command must never throw into the command surface.
        return { text: `mcp: could not read status (${String(error?.message ?? error)})` };
      }
    },
  });
}
 ...
 * @param {object} manager - ServerManager.
 * @param {object} cache - MetadataCache.
 * @returns {string} markdown status.
 */
function renderStatus(manager, cache) {
  const rows = manager.status();
  if (rows.length === 0) {
    return '### 🔌 MCP Adapter\n\nNo MCP servers are configured. Add them to `mcp.json` (see the plugin README).';
  }
  const lines = ['### 🔌 MCP Adapter status', ''];
  for (const row of rows) {
    const definition = manager.getDefinition(row.name);
    const cached = cache.getTools(row.name).length;
    const state = row.disabled
      ? '⚪ disabled'
      : row.breakerOpen
        ? `🔴 unavailable (${row.failures} consecutive failures)`
        : row.connected
          ? '🟢 connected'
          : '⚫ idle (connects on first use)';
    const promoted = definition?.directTools === true
      ? 'all'
      : Array.isArray(definition?.directTools) && definition.directTools.length > 0
        ? definition.directTools.join(', ')
        : 'none';
    lines.push(`- **${row.name}** [${row.transport}] — ${state}`);
    lines.push(`  • tools: ${row.toolCount ?? cached ?? 0}${cached ? ` (${cached} cached for offline search)` : ''}`);
    lines.push(`  • promoted as direct tools: ${promoted}`);
  }
  lines.push('', '`/mcp refresh` re-reads every server\'s tool catalog.');
  return lines.join('\n');
}
