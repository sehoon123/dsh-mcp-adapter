...
/** Hard ceiling on a single unframed line, to bound memory against a runaway server. */
...
export class StdioTransport {
  ...
  async start() {
    ...
    // asynchronously on the stream. Swallow it here and let _die handle state.
    child.stdin.on('error', (error) => this._die(error));

    child.on('error', (error) => this._die(error));
  ...
  _onStdout(chunk) {
    ...
    if (this._stdoutBuffer.length > MAX_LINE_BYTES) {
      ...
      this._die(new RpcClosedError('server stdout exceeded the maximum line size without framing'));
      return;
    }
